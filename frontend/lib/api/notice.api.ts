...
export const noticeApi = {
  ...
  async getNotices(page: number = 1): Promise<PaginatedResponse<Notice>> {
    const response = await safeFetch(`${API_BASE_URL}/notices?page=${page}`, {
      method: 'GET',
  ...
  async getStudentsForNotice(search: string = '', page: number = 1): Promise<PaginatedResponse<StudentForNotice>> {
    ...
    const response = await safeFetch(`${API_BASE_URL}/notices-create/students?${params.toString()}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async getStaffForNotice(search: string = '', page: number = 1): Promise<PaginatedResponse<StaffForNotice>> {
    ...
    const response = await safeFetch(`${API_BASE_URL}/notices-create/staff?${params.toString()}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async getBlocksForNotice(search: string = '', page: number = 1): Promise<PaginatedResponse<BlockForNotice>> {
    ...
    const response = await safeFetch(`${API_BASE_URL}/notices-create/blocks?${params.toString()}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async getNotice(id: string): Promise<Notice> {
    const response = await safeFetch(`${API_BASE_URL}/notices/${id}`, {
      method: 'GET',
      headers: getAuthHeaders(),
    });
  ...
  async createNotice(noticeData: NoticeFormData): Promise<Notice> {
    ...
    if (hasFiles) {
      ...
      const response = await safeFetch(`${API_BASE_URL}/notices`, {
        method: 'POST',
      ...
      const cleanData = Object.entries(noticeData).reduce((acc: Record<string, unknown>, [key, value]) => {
  ...
  async deleteAttachment(noticeId: string, attachmentId: number): Promise<void> {
    const response = await safeFetch(`${API_BASE_URL}/notices/${noticeId}/attachments/${attachmentId}`, {
      method: 'DELETE',
      headers: getAuthHeaders(),
    });
