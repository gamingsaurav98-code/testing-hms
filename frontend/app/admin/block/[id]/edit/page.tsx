...
export default function EditBlock() {
  ...
  useEffect(() => {
    const fetchBlockData = async () => {
      try {
        ...
        const blockData: BlockData = await response.json();
        
        setFormData({
          block_name: blockData.block_name || '',
          location: blockData.location || '',
          manager_name: blockData.manager_name || '',
          manager_contact: blockData.manager_contact || '',
          remarks: blockData.remarks || '',
          block_attachment: null,
        });

        // Set existing image preview if available
        if (blockData.block_attachment_url) {
          setImagePreview(blockData.block_attachment_url);
        }
        
      } catch (error) {
        console.error('Error fetching block data:', error);
        setErrors({
          submit: 'Failed to load block data. Please try again.'
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Block updated successfully:', result);
      ...
      setErrors({
        submit: error instanceof Error ? error.message : 'An error occurred while updating the block'
      });
    } finally {
    ...
    <div className="min-h-screen bg-gray-50 px-2 py-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-1">Update block information and management details</p>
        </div>
              ...
              <div>
                ...
                {!imagePreview ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {/* File Info - Show only if new file is selected */}
